...
func Test_handleLinkIssue(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			add:          true,
			expectError:  true,
			errorMessage: "failed to get repo",
			fc: func(fc *fakegithub.FakeClient) {
		...
		},
		{
			...
			add:          true,
			expectError:  true,
			errorMessage: "failed to get issue",
			fc: func(fc *fakegithub.FakeClient) {
				fc.OrgMembers["kubernetes"] = []string{"user"}
			},
		},
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			err := handleLinkIssue(fc, log, tc.event, tc.add)
...
func TestUpdateFixesLine(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			returnedBody := updateFixesLine(tc.body, tc.issues, tc.addLink)
