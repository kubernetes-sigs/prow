...
func (f *FakeClient) ListCollaborators(org, repo string) ([]github.User, error) {
	...
	// Add collaborators from the new permission-based storage
	if perms, exists := f.CollaboratorPermissions[repoKey]; exists {
		for username := range perms {
			result = append(result, github.User{Login: username})
		}
	}

	// Also add collaborators from the legacy list for backward compatibility
