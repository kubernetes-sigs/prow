...
func TestRequirementDiff(t *testing.T) {
	testCases := []struct {
		...
		branchProtectionFunc   func(string, string, string) (bool, error)
	}{
		// Review decision tests
		{
			name:                   "Changes requested blocks when branch protection requires reviews",
			reviewDecision:         githubql.PullRequestReviewDecisionChangesRequested,
			reviewApprovedRequired: false,
			expectedDiff:           50,
			branchProtectionFunc:   func(org, repo, branch string) (bool, error) { return true, nil },
			expectedDescContains:   "PR has changes requested in reviews",
		},
		{
			name:                   "Changes requested blocks when ReviewApprovedRequired is true",
			reviewDecision:         githubql.PullRequestReviewDecisionChangesRequested,
		...
		{
			name:                   "Changes requested does not block when branch protection does not require reviews",
			reviewDecision:         githubql.PullRequestReviewDecisionChangesRequested,
			reviewApprovedRequired: true,
			expectedDiff:           0,
			branchProtectionFunc:   func(org, repo, branch string) (bool, error) { return false, nil },
		...
		// All aspects with branch issue
		{
			...
			branchProtectionFunc: func(org, repo, branch string) (bool, error) {
				return true, nil
			},
			// 2000 (branch) + 1000 (author) + 100 (milestone) + 2 (missing labels) + 1 (forbidden label) + 50 (review) + 3 (contexts: 2 status + 1 checkrun)
			expectedDiff:         3156,
			expectedDescContains: "Merging to branch release-1.0 is forbidden",
		},
		// All aspects with author issue
		{
			...
			// 0 (branch OK) + 1000 (author) + 100 (milestone) + 3 (missing labels) + 2 (forbidden labels) + 0 (review) + 2 (failed contexts: ci/build pending + e2e-tests failed)
			expectedDiff:         1107,
		...
		// All aspects with milestone issue
		{
			...
			// 0 (branch OK) + 0 (author OK) + 100 (milestone) + 1 (missing approved label) + 1 (forbidden hold label) + 50 (changes requested) + 1 (failed checkrun)
			expectedDiff:         153,
		...
		//  All aspects with changes requested issue
		{
			...
			// 0 (branch OK) + 0 (author OK) + 0 (milestone OK) + 0 (labels OK) + 50 (changes requested) + 0 (contexts OK)
			expectedDiff:         50,
			expectedDescContains: "changes requested",
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			desc, diff := requirementDiff(pr, query, cc, tc.branchProtectionFunc)
