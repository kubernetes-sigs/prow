...
func TestRequirementDiff(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Create tide config with enforce_github_merge_blocks enabled globally
			// This ensures the existing test behavior is preserved
			tide := &config.Tide{
				EnforceGitHubMergeBlocksMap: map[string]bool{
					"*": true,
				},
			}

			desc, diff := requirementDiff(pr, query, cc, tide)
...
func TestEnforceGitHubMergeBlocks(t *testing.T) {
	testCases := []struct {
		name                   string
		org                    string
		repo                   string
		...
		{
			name:             "Enforce enabled globally - BLOCKED state should add diff",
			org:              "test-org",
			repo:             "test-repo",
			mergeStateStatus: "BLOCKED",
			tideConfig: &config.Tide{
		...
		{
			name:             "Enforce disabled globally - BLOCKED state should not add diff",
			org:              "test-org",
			repo:             "test-repo",
			mergeStateStatus: "BLOCKED",
			tideConfig: &config.Tide{
		...
		{
			name:             "Enforce not configured (default false) - BLOCKED state should not add diff",
			org:              "test-org",
			repo:             "test-repo",
			mergeStateStatus: "BLOCKED",
			tideConfig:       &config.Tide{},
		...
		{
			name:             "Enforce enabled for specific org - BLOCKED state should add diff",
			org:              "enforced-org",
			repo:             "test-repo",
			mergeStateStatus: "BLOCKED",
			tideConfig: &config.Tide{
		...
		{
			name:             "Enforce enabled for specific org - different org should not be affected",
			org:              "other-org",
			repo:             "test-repo",
			mergeStateStatus: "BLOCKED",
			tideConfig: &config.Tide{
		...
		{
			name:             "Enforce enabled for specific repo - BLOCKED state should add diff",
			org:              "test-org",
			repo:             "enforced-repo",
			mergeStateStatus: "BLOCKED",
			tideConfig: &config.Tide{
				EnforceGitHubMergeBlocksMap: map[string]bool{
					"test-org/enforced-repo": true,
				},
			},
			expectedDiff:         100,
			expectedDescContains: "Blocked by GitHub",
		...
		{
			name:             "Enforce enabled for specific repo - different repo should not be affected",
			org:              "test-org",
			repo:             "other-repo",
			mergeStateStatus: "BLOCKED",
			tideConfig: &config.Tide{
		...
		{
			name:             "Repo config overrides org config - repo disabled",
			org:              "test-org",
			repo:             "special-repo",
			mergeStateStatus: "BLOCKED",
			tideConfig: &config.Tide{
				EnforceGitHubMergeBlocksMap: map[string]bool{
					"test-org":             true,
					"test-org/special-repo": false,
				},
			},
			expectedDiff: 0,
		},
		{
			name:             "Repo config overrides org config - repo enabled",
			org:              "test-org",
			repo:             "special-repo",
			mergeStateStatus: "BLOCKED",
			tideConfig: &config.Tide{
		...
		{
			name:             "CLEAN merge state - enforce enabled should not add diff",
			org:              "test-org",
			repo:             "test-repo",
			mergeStateStatus: "CLEAN",
			tideConfig: &config.Tide{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			desc, diff := requirementDiff(pr, query, cc, tc.tideConfig)
