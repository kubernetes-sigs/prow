...
func TestRequirementDiff(t *testing.T) {
	testCases := []struct {
		...
		// MergeStateStatus tests
		{
			...
			expectedDiff:           50,
			expectedDescContains:   "PR is blocked by GitHub (check branch protection, reviews, or rulesets)",
		},
		{
		...
		// All aspects with branch issue
		{
			...
			mergeStateStatus: "BLOCKED",
			// 2000 (branch) + 1000 (author) + 100 (milestone) + 2 (missing labels) + 1 (forbidden label) + 50 (blocked) + 3 (contexts: 2 status + 1 checkrun)
			expectedDiff:         3156,
			expectedDescContains: "Merging to branch release-1.0 is forbidden",
		},
		...
		// All aspects with milestone issue
		{
			...
			mergeStateStatus: "BLOCKED",
			// 0 (branch OK) + 0 (author OK) + 100 (milestone) + 1 (missing approved label) + 1 (forbidden hold label) + 50 (blocked) + 1 (failed checkrun)
			expectedDiff:         153,
		...
		//  All aspects with changes requested issue
		{
			...
			mergeStateStatus: "BLOCKED",
			// 0 (branch OK) + 0 (author OK) + 0 (milestone OK) + 0 (labels OK) + 50 (blocked) + 0 (contexts OK)
			expectedDiff:         50,
			expectedDescContains: "PR is blocked by GitHub (check branch protection, reviews, or rulesets)",
		},
	}
