...
func TestReadRepo(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			goal:     "sigs.k8s.io2",
			wd:       path.Join(dir, "find_from_local", "go/src/sigs.k8s.io2"),
			expected: path.Join(dir, "find_from_local", "go/src/sigs.k8s.io2"),
		},
		{
		...
		},
		{
			...
			dirs: []string{
				path.Join(dir, "find_from_explicit_gopath", "src", "sigs.k8s.io2"),
				path.Join(dir, "find_from_explicit_gopath_random", "src", "test-infra2"),
			},
			expected: path.Join(dir, "find_from_explicit_gopath", "src", "sigs.k8s.io2"),
		},
		{
			name:   "prefer gopath",
			goal:   "sigs.k8s.io2",
			...
			dirs: []string{
				path.Join(dir, "prefer_gopath", "src", "sigs.k8s.io2"),
				path.Join(dir, "prefer_gopath", "src", "test-infra2"),
			},
			expected: path.Join(dir, "prefer_gopath", "src", "sigs.k8s.io2"),
		},
		{
			name:   "not exist",
			goal:   "sigs.k8s.io2",
...
func TestFindRepoFromLocal(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			goal:     "sigs.k8s.io",
			wd:       "go/src/sigs.k8s.io",
			expected: "go/src/sigs.k8s.io",
		},
		{
			name: "repo not found",
			goal: "sigs.k8s.io",
